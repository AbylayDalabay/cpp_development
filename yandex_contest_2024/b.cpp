...
void solve(){
    ...
    for(int query = 0;query < q;query++){
        ...
        int find_position = lower_bound(all(ar), st) - ar.begin();

        find_position = find_position + (index - 1);

        if(find_position >= n){
            cout << -1 << en; cn;
        }

        auto res = mismatch(all(st), ar[find_position].begin());

        if(res.f != st.end()){
            cout << -1 << en;
        }else{
            find_position++;
            cout << find_position << en;
        }
    }
}         
